...
public class MotorcyclesServicesImpl implements IMotorcyclesServices, Observable {
    ...
    public MotorcyclesServicesImpl(IUserRepository userRepository, IParticipantRepository participantRepository, IRaceRepository raceRepository, IRegistrationRepository registrationRepository) {
    ...
    @Override
    public void addRace(Race race) throws SQLException, MotorcyclesException {
    ...
    public boolean login2(String username, String password) {
        Long idUser = findAfterUsername(username).getId();
        Optional<User> user = userRepository.findOne(idUser);
        if(user.isPresent()){
    ...
    @Override
    public synchronized boolean login(String username, String password, Observer observer) throws MotorcyclesException {
        Long idUser = findAfterUsername(username).getId();
        Optional<User> user = userRepository.findOne(idUser);
        if(user.isPresent()){
            String hashedPasswordInput = hashPassword(password);
            if (hashedPasswordInput.equals(user.get().getPassword())) {
                if(loggedClients.get(username) != null) {
                    throw new MotorcyclesException("User already logged in.");
                }
    ...
    @Override
    public void notifyAllObservers() throws SQLException, MotorcyclesException {
