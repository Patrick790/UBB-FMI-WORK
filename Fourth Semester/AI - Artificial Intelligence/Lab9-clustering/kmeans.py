...
class KMeansCustom:
    ...
    def fit(self, data):
        ...
        # pentru fiecare iteratie se recalculeaza pozitiile centroizilor ca fiind media tuturor punctelor din cluster
        for _ in range(self.max_iters):
            # calculeaza etichetele pt fiecare esantion din setul de date
            # eticheta este indicele celui mai apropiat centroid
            labels = np.argmin(np.linalg.norm(data[:, None] - self.centroids, axis=2), axis=1)
            new_centroids = [data[labels == i].mean(axis=0) for i in range(self.k) if np.any(labels == i)]
            # se verifica daca pozitiile centroizilor s-au schimbat semnificativ
            if np.allclose(self.centroids, new_centroids):
                break
            self.centroids = new_centroids

    def predict(self, data):
        return np.argmin(np.linalg.norm(data[:, None] - self.centroids, axis=2), axis=1)
...
def emotion_labeling():
    ...
    unsupervised_classifier = KMeansCustom(k=len(label_names))
    unsupervised_classifier.fit(train_features.toarray())

    # pas 5 - testare model
    computed_test_indexes = unsupervised_classifier.predict(test_features.toarray())
